...
public class HelpPanel extends JPanel implements ListSelectionListener
{
	...
	public HelpPanel( Client aClient, File helpDataFile ) throws IOException
	{
		super();
		client = aClient;
	...
	public void loadDataFrom(File helpDataFile) throws IOException
	{
		LineNumberReader br =
			new LineNumberReader (
				new BufferedReader(new FileReader(helpDataFile)));
		String line;
...
//				System.out.println("Skipping comment " + line );
			...
			if ( line.startsWith("---") )
			{
				...
				if ( hitem == null )
				{
					System.out.println("Extra help for non-existent " + group +
						" " + currentEntryName + " ignored");
				}
				else
				{
			...
			if ( line.startsWith("#") )
			{
				if ( currentEntryName != null )
					throw new IOException(
						_("Entry startss before the end of previous one at line ") + br.getLineNumber());
			...
			if ( currentEntryName == null && line.startsWith("@START_") )
			{
				if ( group != DEFAULT_GROUP )
					throw new IOException(
						_("Nested group at line ") + br.getLineNumber());
			...
			if ( line.startsWith("@END_") )
			{
				if ( !group.equals(line.substring("@END_".length()).trim()))
					throw new IOException(
						_("Non current group closed at line ") + br.getLineNumber());
				group = DEFAULT_GROUP;
				continue;
			}

...
abstract class TiledPainter implements Painter {
    ...
    protected void paintBackground(Graphics g, int x, int y, int width, int height) {
    ...
    protected void setBackgroundBufferSize(int w, int h) {
    ...
    private void scrollbackgroundBuffer(int dx, int dy) {
        ...
        if (dx > 0) {
            ...
            clipClearAndPaint(0, 0, dx, m_bufferRect.height);
        } else {
            if (dx < 0) {

                clipClearAndPaint(m_bufferRect.width + dx, 0, -dx, m_bufferRect.height);
            }
        }
        if (dy > 0) {

            clipClearAndPaint(0, 0, m_bufferRect.width, dy);
        } else {
            if (dy < 0) {

                clipClearAndPaint(0, m_bufferRect.height + dy, m_bufferRect.width, -dy);
            }
        }

...
public class City extends PktCityInfo implements FlashingIcon {
    ...
    */
	...
	{
		TileMap tm = new TileMap( (short)5, (short)5,
								getIconWidth(), getIconHeight(), false, false );
		CivMap bigMap;
		JList choiceList;
				...
				public void mouseClicked(MouseEvent e)
				{
					if (e.getClickCount() == 2)
					{
						...
						if ( shield_stock > 0 )
						{
							...
							else if ( obj instanceof PktRulesetBuilding )
							{
								if ( is_building_unit )
								{
									scrap = true;
							...
							if ( scrap )
							{
								int result = JOptionPane.showInternalConfirmDialog(CityDialog.this,
									_("You will be penalized by cutting prod in half\nDo you want to change ?"),
									_("Scrap production"),JOptionPane.YES_NO_OPTION);
			...
			java.util.Arrays.sort(arr, new Comparator() {
				public int compare(Object o1, Object o2)
				{
					 ...
					 if ( b1.is_wonder && !b2.is_wonder )
						return 1;
					 else if ( !b1.is_wonder && b2.is_wonder )
						return -1;
...
*/
	...
	static class ChoiceListRenderer extends DefaultListCellRenderer
	{
			...
			boolean cellHasFocus)
		{
			...
			else if ( value instanceof PktRulesetBuilding )
			{
				PktRulesetBuilding bld = (PktRulesetBuilding)value;
				return
					super.getListCellRendererComponent(list,
						bld.name + " " + bld.build_cost,
						index,isSelected,cellHasFocus);
			}
			else // unit
			{
	...
	static class PopulationDisplay extends JComponent
	{
		...
		public PopulationDisplay( Client c )
		{
			...
			addMouseListener( new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e)
				{
					if ( icons.size() < 1 )
						return;
					int i = (e.getX() - getInsets().left) /
						((Icon)icons.get(0)).getIconWidth();
					if ( i >= types.size() )
						return;
		...
		public void setPeople( int aSize, int happy, int content, int angry,
			int elvis, int sciencist, int taxmen )
		{

...
public class UnitStackDisplay extends JComponent
{
	...
	public Dimension getWantedSize()
	{
		...
		return
			new Dimension(
					unitWidth*horizontalUnits+(horizontalUnits-1)*GAP,
					unitHeight*rows+(rows-1)*GAP
				);
	}
	...
	public void paintComponent(Graphics g)
	{
		...
		for ( int i=0; i < len; i++ )
		{
			...
			u.reallyPaintIcon(this,g,
				ins.left + (i%horizontalUnits)*(unitWidth+GAP) ,
				ins.top + (i/horizontalUnits)*(unitHeight+GAP),
				false
			);

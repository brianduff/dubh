   ...
   public Client()
   {
      ...
      if (ts == null)
      {
         Logger.log(Logger.LOG_NORMAL,
            "No tileset was specified. You can specify a tileset using the "+
            PROP_FREECIV_TILESET+" Java system property."
         );

         Logger.log(Logger.LOG_NORMAL, "Trying the default, \""+
               DEFAULT_TILESET+"\"."
         );
         ts = DEFAULT_TILESET;
      }
...
//      cacheDir = new File(cacheDir, currentTileset);
   ...
   {
      if (m_rulesetManager == null) m_rulesetManager = new RulesetManager(this);
   ...
   {
      if (m_dlgManager == null) m_dlgManager = new DialogManager(this);
      return m_dlgManager;
   }
   ...
   public void setupComponents()
   {
      if (!m_bSetup)
      {
         ...
         splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
            true, mapPanel, m_upMessages.getMainPanel()
         );
      ...
      for (int i=0; i < MENUS.length; i++)
      {
         ...
         for (int j = 1; j < MENUS[i].length; j++)
         {
            ...
            else
            {
               ...
               {
                  menu.add(new ToggleActionMenuItem(
                     (AbstractToggleAction)a
                  ));
               }
               else
               {
                  menu.add(new ActionMenuItem(
                     (AbstractClientAction)a
                  ));
               }
            }
         }
   ...
   public boolean sendToServer(AbstractPacket pack)
   {
      ...
      catch (IOException ioe)
      {
         JOptionPane.showMessageDialog(this,
            _("Error connection to server lost??"),_("Fatal Error"),JOptionPane.ERROR_MESSAGE);
...
/*      unitDescription.setUnit(u);
   ...
   */
   ...
   public void createMap(final int xsize, final int ysize, final boolean isEarth)
   {
      ...
      SwingUtilities.invokeLater(new Runnable() {
         public void run() {
            ...
            map = new CivMap(Client.this, xsize, ysize, getTileSpec().getNormalTileWidth(),
                getTileSpec().getNormalTileHeight(),
                  true, false);
            map.setSize(mapFrame.getContentPane().getSize());
            JScrollPane scroller = new JScrollPane(map);
            scroller.getHorizontalScrollBar().setUnitIncrement(
               getTileSpec().getNormalTileWidth()
            );
            scroller.getHorizontalScrollBar().setBlockIncrement(
               getTileSpec().getNormalTileWidth()*3
            );
            scroller.getVerticalScrollBar().setUnitIncrement(
               getTileSpec().getNormalTileHeight()
            );
            scroller.getVerticalScrollBar().setBlockIncrement(
               getTileSpec().getNormalTileHeight()*3
            );
   ...
   {
      JOptionPane.showMessageDialog(this,
                           e.toString(),_("Fatal Server Connection Error"),JOptionPane.ERROR_MESSAGE);
      System.exit(1); // ??
   }

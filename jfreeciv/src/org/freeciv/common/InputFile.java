...
public class InputFile
{
   ...
   public static final TokenGetter
      INF_TOK_SECTION_NAME = new SectionNameGetter(),
      INF_TOK_ENTRY_NAME = new EntryNameGetter(),
      INF_TOK_EOL = new EOLGetter(),
   ...
   private static class EOLGetter implements TokenGetter
   {
      public String getToken(InputFile inf)
      {
         ...
         if (!inf.atEol())
         {
            ...
            else if (!inf.isComment(inf.m_curLine.charAt(c)))
            {
               return null;
            }
   ...
   private abstract static class WhiteCharGetter implements TokenGetter
   {
      ...
      public String getToken(InputFile inf)
      {
         ...
         if (c >= inf.m_curLine.length()) return null;
   ...
   private static class ValueGetter implements TokenGetter
   {
      public String getToken(InputFile inf)
      {
             ...
             Character.isDigit(ln.charAt(c)))
         {
            ...
            if (c < ln.length()) trailing = ln.charAt(c);
            // *c = '\0';

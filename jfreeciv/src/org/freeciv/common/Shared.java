...
public class Shared
{
   ...
   public static boolean hasCapabilities(String us, String them)
   {
      ...
      {
         if (usTok[i].charAt(0) == '+' && !hasCapability(
            usTok[i].substring(1), them))
         {
            return false;
         }
      }
      ...
      if (!m_bDataInited)
      {
         ...
         for (int toki = 0; toki < allToks.length; toki++)
         {
            ...
            if (tok.charAt(0) == '~')
            {
               if (i > fsLen && !tok.substring(1, 1+fsLen).equals(File.separator))
               {
                  Logger.log(Logger.LOG_NORMAL,
                     "For \""+tok+"\" in data path cannot expand '~'"+
                     " except as '~"+File.separator+"'; ignoring"
                  );
                  i = 0;
               }
               else
               {
                  ...
                  if (home == null)
                  {
                     Logger.log(Logger.LOG_VERBOSE,
                        "Java user.home property is not set, skipping data "+
                        "path component "+tok
                     );
                     i = 0;
                  }
            ...
            if (i != 0)
            {
               ...
               {
                  Logger.log(Logger.LOG_VERBOSE,
                     "Data path component ("+m_alDirs.size()+"):"+tok
                  );
                  m_alDirs.add(tok);
               }
               else
               {
                  Logger.log(Logger.LOG_NORMAL,
                     "\""+tok+"\" in data path component "+
                     (!f.exists() ? "doesn't exist" :
                        (!f.isDirectory() ? "is not a directory" :
                           "is not readable")
                     )+" ignoring"
                  );
               }
   ...
   public static String stripTrailingChars(String orig, String stripChars)
   {
      if (orig == null || stripChars == null)
      {
         throw new IllegalArgumentException(
            "Both orig and stripChars must not be null."
         );
      }
      ...
      if (origLen == 0 || stripLen == 0) return orig;

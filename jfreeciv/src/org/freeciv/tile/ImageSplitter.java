...
public class ImageSplitter implements ImageConsumer {
	...
	public ImageSplitter(int aWidth, int aHeight, int aTileWidth, int aTileHeight) {
		...
		System.out.println("w=" + tileWidth + " h=" + tileHeight + " ht=" + hTiles +
			" vt=" + vTiles);
	}
			...
			for ( int x =0; x<hTiles; x++ )
			{
				Image img = Toolkit.getDefaultToolkit().createImage(
								new FilteredImageSource(
									new MemoryImageSource(tileWidth,tileHeight,model, tiles[x][y], 0, tileWidth)
									,f )
								);
	...
	public FlashingImageIcon[] getIconArray()
	{
		for ( int y=0; y<vTiles; y++ )
			...
			{
				Image img = Toolkit.getDefaultToolkit().createImage(
					new MemoryImageSource(tileWidth,tileHeight,
								model, tiles[x][y], 0, tileWidth));

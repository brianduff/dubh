 ...
 */
	...
	public HexMap(int aHorizontalTiles, int aVerticalTiles,
		int tileSmallLength,
		boolean aHorizontalWrap, boolean aVerticalWrap,
		boolean anUpperRowLeft,
		int anIconULXOffset, int anIconULYOffset,
		int anIconBRXOffset)
	{
		super(aHorizontalTiles,aVerticalTiles,tileSmallLength*4,
			tileSmallLength*3,aHorizontalWrap, aVerticalWrap );
		sh = tileSmallLength;
		sh2 = sh*2;
		sh3 = sh*3;
		sh4 = sh*4;
		...
		int[] xcoords = new int[] {  0,sh2,sh4-1,sh4-1,sh2,  0};
		int[] ycoords = new int[] { sh,  0, sh,sh3-1,sh4-1,sh3-1};
	...
	protected void paintTilesByStack(Graphics g, Rectangle r)
	{
		...
		for ( int y = r.y; y <= endY; y+=sh3 )
		{
			...
			for ( int x = currentStartX; odd ? (x<currentEndX):(x <= currentEndX); x += sh4 )
			{
				...
				if ( p == null )
					continue;
				paintOneTileStack(g,tiles[p.x][p.y],x-sh2+iconULXoffset,
					y-sh2+iconULYoffset);
			}
			odd = !odd;
		}
			...
			for ( int y = r.y; y <= endY; y+=sh3 )
			{
				...
				for ( int x = currentStartX; (odd ? (x<currentEndX):(x <= currentEndX)); x += sh4 )
				{
					...
					if (layer < l.size() )
					{
						...
						if ( icon != null )
						{
							icon.paintIcon(this,g,x-sh2+iconULXoffset,
								y-sh2+iconULYoffset);
						}
						// optimize ?
						layerLeft = true;
...
//			g.setPaintMode();
	...
	{
		int x = (isLeft(ycell) ? xcell*sh4 : xcell*sh4+sh2) + iconULXoffset
			- upperLeftX;
		while ( x < -sh4 )
			x+= totalWidth;
		...
		while ( y < -sh4 )
			y+= totalHeight;
		repaint( x,y,sh4-iconULXoffset+iconBRXoffset,
			sh5-iconULYoffset);
	}
	...
	public void repaintTwoTilesAround(int xcell, int ycell)
	{
		...
		while ( y < -sh4 )
			y+= totalHeight;
		repaint( x-sh6,y-sh6,sh4+2*sh6-iconULXoffset+iconBRXoffset,
			sh5+2*sh6-iconULYoffset);
	}
...
//		System.out.println(insets);
		...
		g.setClip(
		Math.max(r.x,r.x),
		Math.max(r.y,upperLeftY ),r.width,r.height);
	...
	public static void main(String argv[] ) throws IOException
	{
		...
		final Icon columns =
			new ImageIcon(Toolkit.getDefaultToolkit().createImage(new PngImage("data/hexes/columns.png")));

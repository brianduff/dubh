...
public class TileMap extends GenericMap implements ActionListener {
	...
	public TileMap( int aHorizontalTiles, int aVerticalTiles, int aSingleTileWidth, int aSingleTileHeight,
	...
	public Coords absoluteToTileCoordinates( int x, int y )
	{
		...
		if (!vWrap && (y <0 || y >= totalHeight) )
			return null;
		return new Coords(
					(x/singleTileWidth)%horizontalTiles,
					(y/singleTileHeight)%verticalTiles
				);
	}
	...
	public void weaklyCenterOnTile(int x, int y )
	{
		...
		if ( Math.abs(x-p.x) >= (getWidth()/singleTileWidth)/3 ||
			Math.abs(y-p.y) >= (getHeight()/singleTileHeight)/3 )
			centerOnTile(x,y);
	}
	...
	{
		Coords p = toTileCoordinates(getWidth()/2,
									getHeight()/2);
		if ( p == null )
...
//			Logger.log(Logger.LOG_NORMAL, "There are "+columnsToPaint+" cols to paint");
	...
	public void repaintTiles( int x, int y, int w, int h )
	{
		...
		repaint(startx,starty,
				w*singleTileWidth, h*singleTileHeight);
		int currx = startx;
		int curry = starty;
		if ( hWrap )
		{

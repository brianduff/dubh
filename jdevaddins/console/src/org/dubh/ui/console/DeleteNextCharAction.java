...
/**
 * Overridden delete next char action for the console. Only allows deletion
 * inside the command area.
 *
 * @author Brian.Duff@oracle.com
 */
class DeleteNextCharAction extends ConsoleAction
{

  /**
   * Construct an instance of DeleteNextCharAction
   */
...
// ---------------------------------------------------------------------------
  ...
  public void actionPerformed( ActionEvent ae )
  {
    ...
    try
    {
      if ( dot == mark )
      {
        // Allow deletion only if dot > promptpos + 1
        if ( dot > con.getPromptPosition() + 1 && dot < con.getDocument().getLength() )
        {
          con.getDocument().remove( dot, 1 );
        }
        else
        {
